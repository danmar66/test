...
import MobileGiftModalPreview from "../MobileGiftModalPreview/MobileGiftModalPreview";
import DesktopGiftModalPreview from "../DesktopGiftModalPreview/DesktopGiftModalPreview";
import {useFetcher} from "react-router";
import "../../style.css"

export function CustomizeRewardPickerModal({initialDesign}: {initialDesign: DesignSettings | null }) {
  const fetcher = useFetcher();
  const defaultDesign: DesignSettings = {
    title: "Congratulations! You received a free gift.",
    subtitle: "Select up to 2 gifts for your order.",
    button: "Add to cart",
  ...
  const handleSave = () => {
    ...
    shopify.modal.hide("reward-picker-popup");
  }

  return (
    <>
      <Modal id="reward-picker-popup" variant="max">
              ...
              <s-box background="subdued" padding="large" blockSize="100%">
                <s-stack alignItems={"center"} justifyContent="center">
                  {activeTypePreviewIndex === 0 && (
                    <MobileGiftModalPreview settings={design}/>
                  )}
                  {activeTypePreviewIndex === 1 && (
                    <DesktopGiftModalPreview settings={design}/>
                  )}
                </s-stack>
              </s-box>
